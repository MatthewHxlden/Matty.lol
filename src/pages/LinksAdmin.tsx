...
const LinksAdmin = () => {
  ...
  const resetForm = () => setFormData({ name: "", handle: "", url: "", icon: "Globe", sort_order: 0 });
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-foreground">sudo ./links-admin</span>
              <span className="cursor-blink text-primary">_</span>
            </div>
            <div className="flex items-center justify-between">
              <h1 className="text-3xl md:text-4xl font-bold neon-text">{">"} Links Admin</h1>
          ...
          {(isCreating || editingItem) && (
            <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}>
              <TerminalCard title={editingItem ? "edit_link.sh" : "new_link.sh"}>
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2">
                      <label className="text-sm text-muted-foreground">Handle (e.g. @username)</label>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="space-y-2">
                      <label className="text-sm text-muted-foreground">Icon (Github, Twitter, Linkedin, Youtube, MessageCircle, Globe, Mail)</label>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input type="number" value={formData.sort_order} onChange={(e) => setFormData({ ...formData, sort_order: parseInt(e.target.value) || 0 })} className="w-32" />
                  </div>
          ...
          {isLoading ? (
                ...
                {items.map((item) => (
                  <div key={item.id} className="flex items-center justify-between p-3 border border-border hover:border-primary transition-all">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground truncate">{item.handle} • {item.url}</p>
                    </div>
            ...
            <TerminalCard><div className="text-center py-8 text-muted-foreground"><p>// no links yet</p></div></TerminalCard>
          )}
        </motion.div>
      </div>
    </TerminalLayout>
